...
export const Products: React.FC<{ isOlheiro: boolean, visualMode: ModoVisual }> = ({ isOlheiro, visualMode }) => {
  ...
  const salvar = (e: React.FormEvent) => {
    ...
    notificar(form.id ? "Inventário Atualizado" : "Novo Produto Catalogado");
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-10 pb-40 px-1">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
         <div className="flex bg-[var(--bg-input)] p-1.5 rounded-2xl border border-[var(--border-color)] w-fit">
            <button onClick={() => setActiveTab('catalogo')} className={`px-6 md:px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'catalogo' ? 'bg-[var(--nu-purple)] text-white' : 'text-[var(--text-muted)]'}`}>CATÁLOGO</button>
            <button onClick={() => setActiveTab('form')} className={`px-6 md:px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'form' ? 'bg-[var(--nu-purple)] text-white' : 'text-[var(--text-muted)]'}`}>CADASTRAR</button>
         </div>
         <div className="relative w-full lg:w-96">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" size={16} />
            <input className="nu-input !pl-14 w-full" placeholder="Buscar no Estoque..." value={search} onChange={e=>setSearch(e.target.value)} />
         </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'catalogo' ? (
          <motion.div 
            key="cat" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"
          >
             {products.filter(p => p.nome.toLowerCase().includes(search.toLowerCase())).map(p => (
               <div key={p.id} className="nu-card p-8 text-left group overflow-hidden">
                  <div className="flex justify-between items-start mb-10">
                     <div className="flex-1 min-w-0">
                        <p className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest truncate">{isOlheiro ? 'HIDDEN-SKU' : p.sku}</p>
                     ...
                     <span className="text-[8px] font-black px-3 py-1 border-2 border-[var(--border-color)] rounded-lg uppercase tracking-widest shrink-0 ml-2">{p.raridade}</span>
                  </div>
          ...
          <motion.div 
            ...
            className="flex justify-center pt-4 md:pt-10"
          >
            <form onSubmit={salvar} className="nu-card w-full max-w-2xl p-8 md:p-12 space-y-10 text-left">
              <h2 className="text-2xl font-black uppercase italic flex items-center gap-4">
                <Package size={28} className="text-[var(--nu-purple)]" /> {form.id ? 'EDITAR ITEM' : 'CATALOGAR PRODUTO'}
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
                 <div className="space-y-3">
                    ...
                    <input required className="nu-input w-full font-bold" value={form.nome || ''} onChange={e=>setForm({...form, nome: e.target.value})} placeholder="Ex: Fone Bluetooth X1" />
                 </div>
                 <div className="space-y-3">
              ...
              <div className="flex gap-4">
                 <button type="submit" className="btn-fire flex-1 py-6 text-sm">EFETIVAR CATALOGAÇÃO</button>
                 <button type="button" onClick={() => setActiveTab('catalogo')} className="px-8 py-3 bg-[var(--bg-input)] rounded-xl text-[10px] font-black uppercase">CANCELAR</button>
              </div>
            </form>
          </motion.div>
        )}
